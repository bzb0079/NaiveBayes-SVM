...
for kernel in ('linear', 'poly', 'rbf'):
    for c_value in [0.5, 100, 1000]:
        classifier_linear = svm.SVC(kernel=kernel, C=c_value)
        ...
        print("accuracy of " + kernel + " kernel" + " with " + "C=", c_value, accuracy , "%")
        # try:
        #     with open('result_svm_'+ kernel + '_' + 'Cvalue_' + str(c_value) + '.csv', 'w', encoding="utf-8") as csvfile:
...
fig, ax = plt.subplots()
rects1 = ax.bar(x - width/3, accuracy_value_at_point, width, label='C=0.5')
rects2 = ax.bar(x + width/3, accuracy_value_at_one, width, label='C=100')
rects3 = ax.bar(x + width, accuracy_value_at_two, width, label='C=1000')
# rects4 = ax.bar(x - width, women_means, width, label='C=3')

# Add some text for labels, title and custom x-axis tick labels, etc.
ax.set_ylabel('Accuracy (in %)')
ax.set_title('Accuracy of SVM kernels with C values')
ax.set_xticks(x)
